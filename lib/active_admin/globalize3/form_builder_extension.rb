  ...
  module Globalize3
    module FormBuilderExtension
      ...
      def translated_inputs(name = "Translations", options = {}, &block)
        ...
        locals = options.fetch(:locals, nil)
        form_buffers.last << template.content_tag(:div, class: "activeadmin-translations") do
          template.content_tag(:ul, class: "available-locales") do
            (locals ? locals : auto_sort ? I18n.available_locales.sort : I18n.available_locales).map do |locale|
              template.content_tag(:li) do
                I18n.with_locale(locals ? locals : switch_locale ? locale : I18n.locale) do
